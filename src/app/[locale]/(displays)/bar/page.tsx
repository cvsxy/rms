...
export default function BarDisplayPage() {
  ...
  const fetchOrders = async () => {
    ...
    const barOrders = data.map((order: DisplayOrder) => ({
      ...order, items: order.items.filter((i: DisplayItem) => i.menuItem?.destination === "BAR" && i.status !== "SERVED" && i.status !== "CANCELLED"),
  ...
  return (
    <div className="min-h-dvh bg-gray-900 text-white">
      <header className="bg-gray-800 px-6 py-4 flex items-center justify-between">
        <h1 className="text-xl font-bold">🍹 {t("display.bar")}</h1>
